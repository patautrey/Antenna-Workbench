/* ---------------------------------------------------------
   ...
   Visualizes elevation/azimuth patterns, TOA, gain regions,
   and performance metrics for any antenna module.
--------------------------------------------------------- */
...
/* ---------------------------------------------------------
   ...
   (Approximate analytical lobes for visualization)
--------------------------------------------------------- */
    ...
    return `
        <div class="poster-preview">
            ...
            <p style="margin-top:10px;font-size:13px;color:#aaa;">
                ...
                and performance intuition across antenna types.
            </p>
        </div>
    `;
}
...
function handleCompute(root) {
    ...
    const elevPattern = generateElevationPattern(data.toaDeg, data.gainDB);
    const azPattern = generateAzimuthPattern(data.gainDB, data.fbDB, data.fsDB);

    renderPatternCanvas(elevCanvas, elevPattern, "Elevation Pattern");
    renderPatternCanvas(azCanvas, azPattern, "Azimuth Pattern");
...
export default function initPerformer(root) {
    ...
    if (summaryHost) {
        summaryHost.innerHTML =
            "Enter antenna parameters, then click <strong>Compute Pattern</strong>.";
    }
