/* ---------------------------------------------------------
   Antenna Workbench — Half-Square Designer
   Two-vertical array geometry, spacing, gain, TOA,
   and feedpoint Z estimation
--------------------------------------------------------- */
...
function estimateFeedZ(freqMHz) {
    // Half-squares typically present a moderate feedpoint Z
    return 50 + (freqMHz - 7.1) * 0.6; // crude but useful
}
...
function estimateGain(freqMHz, heightM) {
    const base = 4.5; // typical half-square gain in dBi
    const heightFactor = Math.min(heightM / 12, 1.0) * 1.2; // up to +1.2 dB
    const freqFactor = (freqMHz - 7.1) * 0.04;
...
--------------------------------------------------------- */
function computeHalfSquare(freqMHz) {
    const lambda = wavelength(freqMHz);

    return {
        lambda,
        verticalHeight: lambda / 4,
        topWireLength: lambda / 2,
        spacing: lambda / 2
    };
}
...
/* ---------------------------------------------------------
   SUMMARY BUILDER
--------------------------------------------------------- */
function buildSummary(freqMHz, heightM, H, feedZ, gain, toa) {
    ...
    lines.push(`<strong>Vertical radiator height:</strong> ${round(H.verticalHeight, 2)} m`);
    lines.push(`<strong>Top wire length:</strong> ${round(H.topWireLength, 2)} m`);
    lines.push(`<strong>Spacing between verticals:</strong> ${round(H.spacing, 2)} m`);
    ...
    return `
        <div class="poster-preview">
            ...
            <p style="margin-top:10px;font-size:13px;color:#aaa;">
                Half-squares provide excellent low-angle DX performance with simple construction.
            </p>
        </div>
    `;
}
...
function handleCompute(root) {
    const freqStr = $(root, "#hsq-freq").value;
    const heightStr = $(root, "#hsq-height").value;
    const summaryHost = $(root, "#hsq-summary");
    ...
    const H = computeHalfSquare(freqMHz);
    const feedZ = estimateFeedZ(freqMHz);
    const gain = estimateGain(freqMHz, heightM);
    const toa = estimateTOA(heightM, freqMHz);

    summaryHost.innerHTML = "";
    summaryHost.appendChild(infoBox(buildSummary(freqMHz, heightM, H, feedZ, gain, toa)));

    log("halfsquare-designer", "Computed half-square design", {
        freqMHz,
        heightM,
        H,
        feedZ,
...
/* ---------------------------------------------------------
   MODULE ENTRY POINT
--------------------------------------------------------- */
export default function initHalfSquareDesigner(root) {
    const btn = $(root, "#hsq-compute");
    if (btn) btn.addEventListener("click", () => handleCompute(root));

    const summaryHost = $(root, "#hsq-summary");
    if (summaryHost) {
        summaryHost.innerHTML = "Enter frequency and height, then click <strong>Compute Half-Square</strong>.";
    }

    log("halfsquare-designer", "Module initialized");
}
