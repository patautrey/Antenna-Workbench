...
function estimateGain(freqMHz, heightM) {
    ...
    if (frac < 0.1) return 5.0;
    if (frac < 0.2) return 4.0;
    return 3.0;
}
...
function estimateTOA(freqMHz, heightM) {
    ...
    if (frac < 0.1) return 85;
    if (frac < 0.2) return 75;
    return 65;
}
    ...
    return `
        <div class="poster-preview">
            ...
            <p style="margin-top:10px;font-size:13px;color:#aaa;">
                NVIS dipoles provide strong high-angle radiation for regional coverage.
                Multi-wire reflectors enhance NVIS performance even further.
            </p>
        </div>
    `;
}
...
function handleCompute(root) {
    ...
    const reflEnabledStr = $(root, "#nvis-refl-enabled").value;
    const numStr = $(root, "#nvis-refl-num").value;
    const spacingStr = $(root, "#nvis-refl-spacing").value;
    const offsetStr = $(root, "#nvis-refl-offset").value;
    const reflHeightStr = $(root, "#nvis-refl-height").value;

    const summaryHost = $(root, "#nvis-summary");

    const errors = validate(freqStr, heightStr, reflEnabledStr, numStr, spacingStr, offsetStr, reflHeightStr);
    if (errors.length > 0) {
    ...
    const D = computeNVISDipole(freqMHz, heightM);
    const feedZ = estimateFeedZ(freqMHz, heightM);
    ...
    log("nvis-dipole-designer", "Computed NVIS dipole with reflector", {
        freqMHz,
        heightM,
        feedZ,
        gain,
...
export default function initNVISDipoleDesigner(root) {
    ...
    if (summaryHost) {
        summaryHost.innerHTML =
            "Enter frequency, height, and optional NVIS reflector parameters, then click <strong>Compute NVIS Dipole</strong>.";
    }

    log("nvis-dipole-designer", "Module initialized");
}
