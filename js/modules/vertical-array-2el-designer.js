/* ---------------------------------------------------------
   Antenna Workbench — 2-Element Vertical Array Designer
   Spacing, phasing, gain, F/B, TOA, and feedpoint Z
--------------------------------------------------------- */
...
--------------------------------------------------------- */
const PHASE = {
    "0": { label: "0° (broadside)", gain: 0.0, fb: 0.0 },
    "90": { label: "90° (classic cardioid)", gain: 2.0, fb: 12.0 },
    "180": { label: "180° (end-fire)", gain: 3.5, fb: 18.0 }
};
...
function estimateFeedZ(freqMHz, phaseKey) {
    const base = 36; // typical for phased verticals
    if (phaseKey === "180") return base * 0.9;
    if (phaseKey === "90") return base * 1.1;
    return base;
}
...
function estimateGain(freqMHz, heightM, phaseKey) {
    const base = 2.5; // baseline for two verticals
    const heightFactor = Math.min(heightM / 10, 1.0) * 1.0;
...
/* ---------------------------------------------------------
   SUMMARY BUILDER
--------------------------------------------------------- */
function buildSummary(freqMHz, heightM, phaseKey, A, feedZ, gain, fb, toa) {
    ...
    return `
        <div class="poster-preview">
            ...
            <p style="margin-top:10px;font-size:13px;color:#aaa;">
                Two-element vertical arrays provide excellent low-angle DX performance
                with strong directional control.
            </p>
        </div>
    `;
}
...
function handleCompute(root) {
    const freqStr = $(root, "#va2-freq").value;
    const heightStr = $(root, "#va2-height").value;
    const phaseKey = $(root, "#va2-phase").value;
    const summaryHost = $(root, "#va2-summary");
    ...
    summaryHost.innerHTML = "";
    summaryHost.appendChild(infoBox(buildSummary(freqMHz, heightM, phaseKey, A, feedZ, gain, fb, toa)));

    log("vertical-array-2el-designer", "Computed 2-element vertical array design", {
        freqMHz,
        heightM,
        phaseKey,
        A,
...
/* ---------------------------------------------------------
   MODULE ENTRY POINT
--------------------------------------------------------- */
export default function initVerticalArray2elDesigner(root) {
    const btn = $(root, "#va2-compute");
    if (btn) btn.addEventListener("click", () => handleCompute(root));

    const summaryHost = $(root, "#va2-summary");
    if (summaryHost) {
        summaryHost.innerHTML = "Enter frequency, height, and phasing, then click <strong>Compute 2‑Element Array</strong>.";
    }

    log("vertical-array-2el-designer", "Module initialized");
}
